...
def VNS(Rdn,nodeNum,minEdgeNum,maxEdgeNum):
    ...
    while counter < 10:
        ...
        for i in range(maxEdgeNum-minEdgeNum):
                for edge in G[i].edges:
                    G[i+1].add_edge(edge[0],edge[1])
                for node in G[i+1].nodes:
                    check = True
                    val1 = random.randint(0,nodeNum-1)
                    neighbor_list = [n for n in G[i+1].neighbors(node)]
                    for neighbor in neighbor_list:
                        if neighbor == chr(ord("A") + val1) or chr(ord("A") + val1) == node:
                           check = False
                    
                    if check == True:
                       G[i+1].add_edge(node,chr(ord("A") + val1))
                       break
        ...
        for j in range(maxEdgeNum-minEdgeNum+1):
                InitialSolution(G[j])
                sum = weight(G[j])
    ...
    for i in range(maxEdgeNum-minEdgeNum+1):
                Rdn[i] = RdnSum[i]/10
                

def BF(Rdn, nodeNum, minEdgeNum, maxEdgeNum):
...
while(True):
 minEdgeNum = simpledialog.askstring(title="Test",
                                  prompt="Enter minimum edge number:")

 if minEdgeNum.isdigit():
    minEdgeNum = int(minEdgeNum)
    if minEdgeNum > 0 and minEdgeNum < ((nodeNum*(nodeNum-1))/2):
       break

while(True):
 maxEdgeNum = simpledialog.askstring(title="Test",
                                  prompt="Enter maximum vertex number:")

 if maxEdgeNum.isdigit():
    maxEdgeNum = int(maxEdgeNum)
    if maxEdgeNum > minEdgeNum and maxEdgeNum <= ((nodeNum*(nodeNum-1))/2):
       break
...
VNS(Rdn,nodeNum,minEdgeNum,maxEdgeNum)
BF(Rdn2,nodeNum,minEdgeNum,maxEdgeNum)

plt.scatter(EdgeNum, Rdn, label= "VNS", color= "green", 
                marker= "*", s=30)
plt.scatter(EdgeNum2, Rdn2, label= "Brute Force", color= "blue", 
                marker= ".", s=30)
